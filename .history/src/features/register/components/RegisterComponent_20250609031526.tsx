...
const RegisterComponent = () => {
  ...
  const nextStep = useCallback(() => {
    if (currStep === totalSteps) {
      router.push("/success"); // Navigate to /success on last step
    } else {
      setCurrStep((prev) => (prev < totalSteps ? prev + 1 : prev));
    }
  }, [currStep, router, totalSteps]);
...
const ChangeStep = ({
  ...
  nextStep: () => void;
  prevStep: () => void;
  stepperContainerRef: React.RefObject<HTMLDivElement>;
  ...
  const scrollToCurrentStep = (step: number) => {
    const container = stepperContainerRef.current;
    const currentStepElement = stepRefs.current[step - 1];
  ...
  const handleNext = () => {
    if (currStep === totalSteps) {
      nextStep(); // navigate to success page or final step
      return;
    }
    const newStep = currStep + 1;
    nextStep();
    scrollToCurrentStep(newStep);
  };
