...
import { BANK_DETAILS } from "./constant";
  ...
  return (
    <div className="bg-white rounded-[25px] px-4 md:px-[22px] pb-6 md:pb-[18px] w-full mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-6">
        {BANK_DETAILS.map(({ label, fullForm, key }, index) => (
          <div key={index}>
            ...
            <span className="font-secondary lg:text-[14px] md:text-[10px] text-[#333333] break-all">
              {bankDetails?.[key] || "N/A"}
            </span>
          </div>
        ))}
