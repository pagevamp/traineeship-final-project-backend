...
const Rates = () => {
  ...
  return (
    <div className="w-full flex flex-col items-center gap-4 px-4 sm:px-6 md:px-8">
      ...
      <div className="flex gap-4 sm:gap-8 mt-6 border-b border-[#D3A8FF] w-full max-w-[1110px] flex-wrap sm:flex-nowrap">
        {["FTL", "LTL"].map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab as "FTL" | "LTL")}
            className={`relative text-[16px] font-medium text-[#540F86] pb-2 whitespace-nowrap ${
      ...
      {/* Content container - allow horizontal scroll on small screens */}
