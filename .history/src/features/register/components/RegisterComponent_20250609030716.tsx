...
const RegisterComponent = () => {
  ...
  // Scroll buttons visibility on scroll
  useEffect(() => {
  ...
  // Scroll logic extracted to reusable function
  ...
  // Optional: scroll active step on currStep change (could be removed if handled by buttons)
  useEffect(() => {
    scrollToStep(currStep);
  }, [currStep, scrollToStep]);

  // Next step handler: update state and scroll active step
  ...
  }, [router, totalSteps, scrollToStep]);

  // Previous step handler: update state and scroll active step
  ...
  // Render your step components here (adjust these imports or definitions)
  const renderStep = () => {
    switch (currStep) {
      case 1:
  ...
  return (
    <section className="font-secondary max-w-[964px] mx-auto h-full flex flex-col items-center pt-[34px] relative">
      ...
      >
        {/* We map steps here so we can attach refs to each step */}
        <div className="flex gap-4 min-w-max">
          {steps.map((step, idx) => (
            <div
              ...
              className={`step-item ${currStep === step.id ? "active" : ""}`}
            >
              {/* If your Stepper component renders the steps internally, you may need to modify it
                  or alternatively render it here step-by-step */}
              {/* For now, just render a simple label or you can customize */}
              <Stepper
                steps={[step]} // render one step at a time here for ref
                currentStep={currStep === step.id ? currStep : 0}
              />
            </div>
          ))}
        </div>
...
const RegisterStep5 = () => {
  return (
    <div className="text-[16px] min-w-[320px] max-w-screen-2xl mx-auto px-4 flex flex-col gap-6 md:gap-[21px] mt-20 mb-20">
