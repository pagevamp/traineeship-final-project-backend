        ...
        {slides.map((slide, idx) => (
          <SwiperSlide key={idx}>
            <div className="relative w-full h-full">
              ...
              <div className="absolute inset-0" style={{ backgroundColor: "#540F8630" }}>
                <div className="flex flex-col justify-between items-center text-center h-full p-10 text-white">
                  <div className="mt-10">
                    <h1 className="font-primary text-[40px]">{slide.heading}</h1>
                    <p className="font-secondary text-[22px] mt-2">{slide.subheading}</p>
                  </div>
                  <div className="mb-16">
                    <h2 className="font-primary text-[40px]">{slide.bottomText}</h2>
                    <p className="font-secondary text-[22px] mt-1">{slide.description}</p>
                  </div>
                </div>
              </div>
            </div>
